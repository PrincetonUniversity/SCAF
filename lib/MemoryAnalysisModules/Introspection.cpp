...
cl::opt<std::string> FirstCallee("watch-callee-a", cl::init(""), cl::NotHidden,
                                 cl::desc("First callee name"));
cl::opt<std::string> SecondCallee("watch-callee-b", cl::init(""), cl::NotHidden,
...
cl::opt<std::string> StorePtrName("watch-st-ptr", cl::init(""), cl::NotHidden,
...
void exitIntrospectionRegion() { IntrospectStack.pop_back(); }
...
void pquery(StringRef who, bool enter, const Instruction *i1,
            LoopAA::TemporalRelation Rel, const Instruction *i2, const Loop *L,
  ...
  errs() << (enter ? "Enter " : "Exit ") << who << "::modref(" << *i1 << ", "
         << Rel << ", " << *i2 << ", ";
  if (L)
...
void pquery(StringRef who, bool enter, const Value *p1, unsigned s1,
            LoopAA::TemporalRelation Rel, const Value *p2, unsigned s2,
            const Loop *L, LoopAA::AliasResult res) {
  doIndent(enter);

  if (!enter)
    errs() << "} ";

  errs() << (enter ? "Enter " : "Exit ") << who << "::alias(";
  pptr(p1, s1);
  errs() << ", " << Rel << ", ";
  pptr(p2, s2);
  errs() << ", ";
...
void pquery(StringRef who, bool enter, const CallBase &CS1,
            ...
            LoopAA::ModRefResult res) {
  pquery(who, enter, &CS1, Rel, &CS2, L, res);
}

void pquery(StringRef who, bool enter, const CallBase &CS1,
            LoopAA::TemporalRelation Rel, const ClassicLoopAA::Pointer &P2,
            const Loop *L, LoopAA::ModRefResult res) {
  pquery(who, enter, &CS1, Rel, P2.ptr, P2.size, L, res);
}
