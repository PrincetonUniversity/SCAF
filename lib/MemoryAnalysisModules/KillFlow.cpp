...
bool KillFlow::mustAlias(const Value *storeptr, const Value *loadptr) {
  ...
  Remedies R;
  return top->alias(storeptr, 1, Same, loadptr, 1, 0, R, LoopAA::DMustAlias) ==
         LoopAA::MustAlias;
}

/// Non-topping case of pointer comparison.
bool KillFlow::mustAliasFast(const Value *storeptr, const Value *loadptr,
                             const DataLayout &DL) {
  UO a, b;
...
bool KillFlow::instMustKill(const Instruction *inst, const Value *ptr,
  ...
  if (const StoreInst *store = dyn_cast<StoreInst>(inst)) {
    ...
    if (mustAlias(storeptr, ptr)) {
      LLVM_DEBUG(
          errs()
          << "There can be no loop-carried flow mem deps to because killed by "
          << *store << '\n');
      return true;
    }
  }
          ...
          if (auto exitS = dyn_cast<StoreInst>(&exitI)) {
            if (const GetElementPtrInst *exitGep =
              ...
              if (exitGep->hasAllConstantIndices() &&
                ...
                if (firstIdxC && firstIdxC->getZExtValue() == 0 && secondIdxC &&
      ...
      if ((!zeroInitialized || srcs.size() != 1)) {
        ...
        if (noStoresBetween.count(ikey)) {
          ...
          if (!noExtNumOfElemI || !noExtKillLimitI)
            return false;
          noStoreBetween =
              noStoreInBetween(noExtNumOfElemI, noExtKillLimitI, srcs, *mloops);
...
bool KillFlow::matchingIdx(const Value *idx, const Value *killidx,
  ...
  if (se->isSCEVable(killidx->getType())) {
    ...
    if (se->isSCEVable(idx->getType()))
      addRec = dyn_cast<SCEVAddRecExpr>(se->getSCEV(const_cast<Value *>(idx)));
    ...
    if (killAddRec && addRec) {
      ...
      if (const SCEVConstant *startDiffConst =
              dyn_cast<SCEVConstant>(startDiff)) {
      ...
      if (const SCEVConstant *stepDiffConst =
              dyn_cast<SCEVConstant>(stepDiff)) {
...
/// If <after> belongs to this block and <after> is not null, only consider
/// operations AFTER <after> If <after> belongs to this block and <before> is is
/// not null, only consider operations BEFORE <before>
bool KillFlow::blockMustKill(const BasicBlock *bb, const Value *ptr,
                             const Instruction *after,
                             const Instruction *before, time_t queryStart,
                             unsigned Timeout, const Loop *L) {
  ...
  if (bb != beforebb && bb != afterbb && L) {
    ...
    if (li->isLoopHeader(bb)) {
      ...
      for (auto loopBB : innerLoop->getBlocks()) {
        ...
        for (BasicBlock::const_iterator k = loopBB->begin(), e = loopBB->end();
             k != e; ++k) {
  ...
  */
  ...
  if (Rel == Same) {
    if (L->contains(i1) && L->contains(i2)) {
      ...
      // intra-iteration mem dep query. No need to check again here
      /*
  ...
  if (L->contains(later)) {
    if (const LoadInst *load = dyn_cast<LoadInst>(later)) {
      ++numEligibleBackwardLoadQueries;
      // if( pointerKilledBefore(L, load->getPointerOperand(), load, queryStart,
      // AnalysisTimeout) )
    ...
    if (const StoreInst *store = dyn_cast<StoreInst>(later)) {
      ++numEligibleBackwardStoreQueries;
      // if( pointerKilledBefore(L, store->getPointerOperand(), store,
      // queryStart, AnalysisTimeout) )
      if (pointerKilledBefore(L, laterPtr, store, queryStart,
                              AnalysisTimeout)) {
        ++numKilledBackwardStore;
        // no dependence between this store and insts from previous iteration
        // possible
        res = NoModRef;
      }
  ...
  if (L->contains(earlier)) {
    if (const StoreInst *store = dyn_cast<StoreInst>(earlier)) {
      ++numEligibleForwardStoreQueries;
      // if( pointerKilledAfter(L, store->getPointerOperand(), store,
      // queryStart, AnalysisTimeout) )
      if (pointerKilledAfter(L, earlierPtr, store, queryStart,
                             AnalysisTimeout)) {
        ++numKilledForwardStoreFlows;
        // res = ModRefResult(res & ~Mod);
        res = NoModRef;
    ...
    if (const LoadInst *load = dyn_cast<LoadInst>(earlier)) {
      ++numEligibleForwardLoadQueries;
      // if( pointerKilledAfter(L, load->getPointerOperand(), load, queryStart,
      // AnalysisTimeout) )
      if (pointerKilledAfter(L, earlierPtr, load, queryStart,
                             AnalysisTimeout)) {
  ...
  LLVM_DEBUG(errs() << "Can't say jack about " << *i2 << " at "
                    << i2->getParent()->getParent()->getName() << ':'
                    << i2->getParent()->getName() << '\n');
  INTROSPECT(EXIT(i1, Rel, i2, L, res));
  if (res != NoModRef) {
    Remedies tmpR;
    ModRefResult nextRes = getEffectiveNextAA()->modref(i1, Rel, i2, L, tmpR);
...
bool KillFlow::blockMustKillAggregate(const BasicBlock *bb,
...
/// Determine if there is an operation in <L> which must execute before <before>
/// which kills <ptr>
bool KillFlow::pointerKilledBefore(const Loop *L, const Value *ptr,
                                   const Instruction *before,
  ...
  if (!start) {
    errs() << "The dominator tree does not contain basic block: "
           << beforebb->getName() << '\n'
...
bool KillFlow::pointerKilledBetween(const Loop *L, const Value *ptr,
                                    const Instruction *after,
                                    const Instruction *before,
                                    bool alsoCheckAggregate, time_t queryStart,
                                    unsigned Timeout) {
  ...
  if (alsoCheckAggregate) {
    ...
    if (aggregateKilledBetween(L, aggregate, after, before, queryStart,
                               Timeout))
      return true;
  }
...
/// which kills <ptr>
bool KillFlow::pointerKilledAfter(const Loop *L, const Value *ptr,
                                  const Instruction *after,
                                  bool alsoCheckAggregate, time_t queryStart,
                                  unsigned Timeout) {
  if (!ptr)
  ...
  if (!start) {
    errs() << "The post-dominator tree does not contain basic block: "
           << afterbb->getName() << '\n'
           << "  (this probably means that the block is located in an infinite "
              "loop)\n";
...
bool KillFlow::aggregateKilledBetween(const Loop *L, const Value *obj,
                                      const Instruction *after,
                                      const Instruction *before,
                                      time_t queryStart, unsigned Timeout) {
  if (!obj)
...
static RegisterPass<KillFlow>
    X("kill-flow-aa",
      "Reasons about operations which kill data flow between loop iterations.");
