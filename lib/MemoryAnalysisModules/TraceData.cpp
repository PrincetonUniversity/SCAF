...
STATISTIC(numTraceValueSteps,
          "Num invocations of traceConcreteFunctionPointers()");
STATISTIC(numTraceLoadFromSteps,
          "Num invocations of traceConcreteFunctionPointersLoadedFrom()");
STATISTIC(numTraceConstantInitializerSteps,
          "Num invocations of "
          "extractConcreteFunctionPointersFromConstantInitializer()");
...
bool Tracer::traceConcreteIntegerValues(Value *expr, IntSet &output) const {
  ...
  if (traceConcreteIntegerValues(expr, output, already)) {
    LLVM_DEBUG(if (!output.empty()) {
      errs() << "Value " << *expr << " can only take the values: ";
      for (IntSet::const_iterator i = output.begin(), e = output.end(); i != e;
           ++i)
        errs() << *i << ", ";
      errs() << '\n';
    });
...
bool Tracer::traceConcreteIntegerValues(Value *expr, IntSet &output,
  ...
  if (isa<UndefValue>(expr))
    ...
    return traceConcreteIntegerValues(sel->getTrueValue(), output, already) &&
  ...
  */
  ...
  // TODO: load from constant memory
    ...
    for (IntSet::const_iterator i = vals2.begin(), e = vals2.end(); i != e;
         ++i) {
      const uint64_t vi = *i;
  ...
  LLVM_DEBUG(errs() << "traceConcreteIntegerValues() failed on: ``" << *expr
                    << "''\n");
  return false;
}
...
bool Tracer::traceConcreteFunctionPointers(Value *fcn_ptr, FcnList &output,
  ...
  ++numTraces;
  const bool success =
      traceConcreteFunctionPointers(fcn_ptr, output, already, DL);
...
bool Tracer::traceConcreteFunctionPointers(Value *fcn_ptr, FcnList &output,
  ...
  if (Function *fcn = dyn_cast<Function>(fcn_ptr)) {
    ...
    return traceConcreteFunctionPointersLoadedFrom(load->getPointerOperand(),
                                                   output, already, DL);
  ...
  if (!uo.empty() && fcn_ptr != *uo.begin()) // if progress
  {
    for (UO::iterator i = uo.begin(), e = uo.end(); i != e; ++i)
      if (!traceConcreteFunctionPointers(const_cast<Value *>(*i), output,
                                         already, DL))
        return false;
    return true;
  }

  LLVM_DEBUG(errs() << "Flow-sensitive trace failed on: ``" << *fcn_ptr
                    << "''\n");
  return false;
}
...
bool Tracer::traceConcreteFunctionPointersLoadedFrom(
  ...
  // A special case to make this analysis partially field- and
  // element-sensitive.
  ...
  if (!uo.empty() && ptr != *uo.begin()) // if progress
  {
    ...
    for (UO::iterator i = uo.begin(), e = uo.end(); i != e && all_good; ++i)
      if (!traceConcreteFunctionPointersLoadedFrom(const_cast<Value *>(*i),
                                                   output, already, DL))
        all_good = false;
  ...
  if (GetElementPtrInst *gep = dyn_cast<GetElementPtrInst>(ptr)) {
    ...
    if (noescape.isFieldPointer(gep, &structty, &fieldno)) {
      ...
      if (noescape.findAllDefs(structty, fieldno, defs) && !defs.empty()) {
        ...
        LLVM_DEBUG(errs() << "Load from non-escaping field:\n"
                          << " GEP: " << *gep << '\n'
                          << " Structure: " << *structty << '\n'
                          << " Field: " << *fieldno << '\n'
                          << "Definitions:\n";
                   for (I i = defs.begin(), e = defs.end(); i != e; ++i) errs()
                   << " o " << *i->second << '\n';);
  ...
  LLVM_DEBUG(errs() << "Flow-sensitive trace load-from failed on: ``" << *ptr
                    << "''\n");
  return false;
}
...
bool Tracer::extractConcreteFunctionPointersFromConstantInitializer(
  ...
  else if (const ConstantArray *ca = dyn_cast<ConstantArray>(constant)) {
  ...
  // Other fields within a constant which cannot hold a pointer address.
    ...
    if (cexpr->isCast() || cexpr->getOpcode() == Instruction::GetElementPtr) {
