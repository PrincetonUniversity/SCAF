...
bool NoCaptureFcn::runOnModule(Module &mod) {
  ...
  LLVM_DEBUG(
      ...
      unsigned total = 0; for (MinArity2FcnList::iterator i = captured.begin(),
                               e = captured.end();
                               i != e; ++i) {
...
NoCaptureFcn::iterator NoCaptureFcn::begin(unsigned arity) const {
...
static RegisterPass<NoCaptureFcn>
    mp("no-capture-fcn", "Analyze functions whose address is never captured.");
} // namespace liberty
