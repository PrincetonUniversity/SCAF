...
namespace {
static RegisterPass<AAvsOracle_EarlyHelper> X("aa-vs-oracle-early",
                                              "Early helper for AA-vs-Oracle");
static RegisterPass<AAvsOracle> Y("aa-vs-oracle",
                                  "Compare ::Alias-Analysis to Oracle");
} // namespace

bool AAvsOracle_EarlyHelper::gather(Function *oracle, Truths &collection,
                                    Module &mod) {
  bool modified = false;
  ...
  for (ATI i = begin; i != end; ++i) {
    ...
    case llvm::AliasResult::NoAlias:
      ...
      if (truth == AA_MAY || truth == AA_MUST) {
        fprintf(stderr,
                "ERROR: Line %d \"%s\": Oracle says May/Must, but AA reports "
                "No Alias.\n",
                i->line, i->desc.c_str());
      }
      break;
    ...
    case llvm::AliasResult::PartialAlias:
      ...
      if (truth != AA_MAY) {
        fprintf(
            stderr,
            "Imprecise: Line %d \"%s\": Oracle=%d, but AA reports May-Alias.\n",
    ...
    case llvm::AliasResult::MustAlias:
      ...
      if (truth != AA_MUST) {
        fprintf(stderr,
                "Imprecise: Line %d \"%s\": Oracle=%d, but AA reports "
                "Must-Alias.\n",
                i->line, i->desc.c_str(), truth);
      }
      break;
...
bool AAvsOracle::runOnModule(Module &mod) {
  ...
  test(AA_MUST, helper.must_alias_begin(), helper.must_alias_end(),
       stats[AA_MUST]);
  ...
  fprintf(stderr, "Oracle-Must:    %3d       %3d        %3d\n",
          stats[AA_MUST][AA_NO], stats[AA_MUST][AA_MAY],
          stats[AA_MUST][AA_MUST]);

  fprintf(stderr, "\n");

  unsigned precise =
      stats[AA_NO][AA_NO] + stats[AA_MAY][AA_MAY] + stats[AA_MUST][AA_MUST];
  unsigned conservative = stats[AA_NO][AA_MAY] + stats[AA_NO][AA_MUST] +
                          stats[AA_MAY][AA_MUST] + stats[AA_MUST][AA_MAY];
