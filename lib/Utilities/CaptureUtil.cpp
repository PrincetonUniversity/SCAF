...
static bool findAllCapturesRec(const Value *v, VisitedSet &visited,
                               CaptureSet *captureSet);

static bool captures(const Value *v, const Value *use, VisitedSet &visited,
                     CaptureSet *captureSet) {
  ...
  for (unsigned i = 0; i < CS->arg_size(); ++i) {
    if (CS->getArgOperand(i) == v &&
      ...
      if (f->isVarArg() || f->isDeclaration() ||
        ...
        LLVM_DEBUG(errs() << "Captured " << *GetUnderlyingObject(v, DL)
