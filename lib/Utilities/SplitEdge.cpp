...
static BasicBlock *fixLandingPad(BasicBlock *from, BasicBlock *to,
                                 StringRef prefix, InvokeInst *invoke,
  ...
  BB2BB new_tos;
  for (Value::user_iterator i = to->user_begin(), e = to->user_end(); i != e;
       ++i) {
    ...
    if (new_tos.count(pred))
      ...
      BasicBlock *new_to =
          CloneBasicBlock(to, vmap, ".critical-lpad.", to->getParent());
      new_tos[pred] = new_to;
  ...
  for (BB2BB::iterator i = new_tos.begin(), e = new_tos.end(); i != e; ++i) {
    ...
    for (BasicBlock::iterator j = new_to->begin(), z = new_to->end(); j != z;
         ++j) {
...
BasicBlock *split(BasicBlock *from, unsigned succno, StringRef prefix,
  ...
  if (InvokeInst *invoke = dyn_cast<InvokeInst>(term))
    if (invoke->getUnwindDest() == to)
      return fixLandingPad(from, to, prefix, invoke, updateLoopInfo,
                           destination_loop);
