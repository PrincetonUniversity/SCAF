...
public:
  ...
  enum DesiredAliasResult { DNoAlias = 1, DMustAlias = 2, DNoOrMustAlias = 3 };
  ...
  enum SchedulingPreference { Bottom = 1, Low = 25, Normal = 50, Top = 100 };
  ...
  /// The result is a one of:
  ///
  ///   NoModRef :      A neither reads nor writes B's memory when I(A) rel
  ///   I(B). Ref      :      A may read B's memory when I(A) rel I(B). Mod : A
  ///   may write B's memory when I(A) rel I(B). ModRef   :      A may read or
  ///   write B's memory when I(A) rel I(B).
  ///
  /// Also, when implementations of LoopAA chain to
  /// lower LoopAAs, they call this method in the
  /// base class.
  ...
  bool canBasicBlockModify(const BasicBlock &BB, TemporalRelation Rel,
  ...
  bool canInstructionRangeModify(const Instruction &I1, TemporalRelation Rel,
                                 const Instruction &I2, const Value *Ptr,
                                 unsigned Size, const Loop *L, Remedies &R);

  bool mayModInterIteration(const llvm::Instruction *A,
                            const llvm::Instruction *B, const llvm::Loop *L,
                            Remedies &R);
  ...
  bool isNoAlias(const Value *V1, unsigned V1Size, TemporalRelation Rel,
                 const Value *V2, unsigned V2Size, const Loop *L, Remedies &R) {
    Remedies tmpR;
  ...
  void InitializeLoopAA(Pass *P, Module &tli, const DataLayout &t);

  /// Alternatively, send this.
  /// Subclasses must call this method at the beginning
  /// of runOnWhatever() to initalize the the LoopAA interface.
  void InitializeLoopAA(const DataLayout *t, TargetLibraryInfo *tli,
                        LoopAA *next);
  ...
  ModRefResult chain(Remedies &finalRemeds, const Instruction *A,
                     TemporalRelation rel, const Instruction *B, const Loop *L,
                     ModRefResult curRes, Remedies &curRemeds);
  ModRefResult chain(Remedies &finalRemeds, const Instruction *A,
...
private:
  ...
  unsigned totals[2][4], fcnTotals[2][4], loopTotals[2][4];
};
